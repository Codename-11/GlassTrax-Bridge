...
import { configApi, adminApi, dsnsApi, testDsnApi, testAgentApi, getErrorMessage, type ConfigData } from '@/lib/api';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" onClick={handleReset} disabled={!isDirty || saveMutation.isPending}>
            Reset
          </Button>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="dsn">
                ...
                <Badge variant="outline" className="ml-2 text-xs">Restart Required</Badge>
              </Label>
                    ...
                    {dsnsData?.dsns.map((dsn) => (
                      <SelectItem key={dsn.name} value={dsn.name}>
                        <div className="flex items-center gap-2">
                          <span>{dsn.name}</span>
                          {dsn.is_pervasive && (
                            <span className="text-xs text-green-600 font-medium">(Pervasive)</span>
                          )}
                        </div>
                      </SelectItem>
                    ))}
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                This setting cannot be changed through the UI.
              </p>
            </div>
            <Switch
              checked={formData.database.readonly}
              disabled={true}
            />
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">IANA timezone name</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => updateApplication('performance.query_timeout', parseInt(e.target.value) || 60)}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => updateApplication('performance.fetch_size', parseInt(e.target.value) || 1000)}
              />
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              When enabled, the API connects to a Windows-hosted agent for GlassTrax
              database queries. The agent handles ODBC connections to Pervasive SQL.
            </AlertDescription>
          </Alert>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="pt-2">
                <Button
                  ...
                  disabled={!formData.agent.url || !formData.agent.api_key || testAgentMutation.isPending}
                >
              ...
              {agentTestResult && (
                <Alert variant={agentTestResult.connected ? 'success' : 'destructive'}>
                  ...
                  <AlertTitle>{agentTestResult.connected ? 'Connected' : 'Connection Failed'}</AlertTitle>
                  <AlertDescription>
                    {agentTestResult.message}
                    {agentTestResult.agent_version && (
                      <span className="block text-xs mt-1">Agent version: {agentTestResult.agent_version}</span>
                    )}
                  </AlertDescription>
                </Alert>
              )}
          ...
          <div className="text-sm text-muted-foreground">
            <p className="font-medium mb-2">Agent Setup:</p>
            <ol className="list-decimal list-inside space-y-1 text-xs">
              <li>On Windows, start the agent: <code className="bg-muted px-1 py-0.5 rounded">.\agent\run_agent.bat</code></li>
              <li>Copy the API key shown on first run</li>
              <li>Enable agent mode above and enter the URL and API key</li>
              <li>For Docker: <code className="bg-muted px-1 py-0.5 rounded">AGENT_ENABLED=true AGENT_URL=http://192.168.1.100:8001 docker-compose up -d</code></li>
            </ol>
            <p className="mt-2">
              <a
