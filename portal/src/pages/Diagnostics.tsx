...
export function DiagnosticsPage() {
  ...
  // Collapsible state (only diagnostics is collapsible)
  ...
  const connectionMode = config?.agent?.enabled ? 'Agent' : 'Direct ODBC'
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">System health, performance, and server controls</p>
        </div>
      </div>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-4">
            <div className="rounded-lg border p-3">
              <div className="text-muted-foreground text-xs">Connection Mode</div>
              <div className="text-lg font-semibold">{connectionMode}</div>
            </div>
            <div className="rounded-lg border p-3">
              <div className="text-muted-foreground text-xs">Database</div>
              <div className="text-lg font-semibold">
                {config?.database?.friendly_name || 'Not configured'}
              </div>
            </div>
            <div className="rounded-lg border p-3">
              <div className="text-muted-foreground text-xs">Version</div>
              <div className="text-lg font-semibold">{health?.version || 'Unknown'}</div>
            </div>
            <div className="rounded-lg border p-3">
              <div className="text-muted-foreground text-xs">Timezone</div>
              <div className="text-lg font-semibold">{config?.application?.timezone || 'UTC'}</div>
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Diagnostic Checks - Collapsible */}
          ...
          <CollapsibleTrigger asChild>
            <CardHeader className="hover:bg-muted/50 cursor-pointer">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <ChevronIcon
                    className={`h-4 w-4 transition-transform ${diagnosticsOpen ? 'rotate-90' : ''}`}
                  />
                ...
                <Button
                  ...
                  disabled={isFetching}
                >
                  {isFetching ? 'Running...' : diagnostics ? 'Re-run' : 'Run Diagnostics'}
                </Button>
              </div>
            </CardHeader>
          </CollapsibleTrigger>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              Speed Test
              {speedTestResult && <Badge variant="outline">{speedTestResult.total_ms}ms</Badge>}
            </CardTitle>
            <Button
              size="sm"
            ...
            >
              {speedTestMutation.isPending ? 'Running...' : 'Run Test'}
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          {speedTestResult ? (
            ...
            <div className="text-muted-foreground py-4 text-center text-sm">
              Click "Run Test" to measure database latency
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                ...
                {cacheStatus && <Badge variant="secondary">{cacheStatus.entries} cached</Badge>}
              </CardTitle>
              <div className="flex gap-2">
                ...
                >
                  {isFetchingCache ? 'Loading...' : cacheStatus ? 'Refresh' : 'Load'}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  onClick={() => clearCacheMutation.mutate()}
                  disabled={clearCacheMutation.isPending || !cacheStatus?.entries}
                >
              ...
              <>
                <div className="grid gap-4 md:grid-cols-4">
                  <div className="rounded-lg border p-3">
                    ...
                    <div className="text-2xl font-bold">{cacheStatus.entries}</div>
                    <div className="text-muted-foreground text-xs">
                      max {config?.caching?.max_cached_dates ?? 7}
                    </div>
                  </div>
                  <div className="rounded-lg border p-3">
                    <div className="text-muted-foreground text-xs">TTL</div>
                    <div className="text-2xl font-bold">
                      {config?.caching?.fabs_ttl_minutes ?? 30}
                    </div>
                ...
                {cacheStatus.hit_rate !== null && (
                  <div className="text-muted-foreground mt-4 text-xs">
                    Hit Rate: {cacheStatus.hit_rate}%
                  </div>
                )}
              </>
            ) : (
              <div className="text-muted-foreground py-4 text-center text-sm">
                Click "Load" to view cache statistics
              </div>
            )}
          </CardContent>
        </Card>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-red-600">Reset Application Database</DialogTitle>
            <DialogDescription>
              This action cannot be undone. This will permanently delete all API keys, applications,
              and access logs. A new admin key will be generated when you restart the server.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <label className="text-muted-foreground mb-2 block text-sm">
              Type <code className="bg-muted rounded px-1 font-bold">RESET DATABASE</code> to
              confirm:
            </label>
            <Input
              value={confirmText}
