...
<#
.SYNOPSIS
    Builds GlassTrax API Agent Windows installer

.DESCRIPTION
    This script builds a standalone Windows installer for the GlassTrax API Agent.
...
Write-Host "  GlassTrax API Agent Build Script" -ForegroundColor Cyan
...
$InstallerContent = @"
; GlassTrax API Agent Installer
; Copyright (c) 2025-2026 Axiom-Labs. All Rights Reserved.
; Generated by build_agent.ps1
; Version: $Version

#define MyAppName "GlassTrax API Agent"
#define MyAppVersion "$Version"
#define MyAppPublisher "Axiom-Labs"
#define MyAppCopyright "Copyright (c) 2025-2026 Axiom-Labs"
...
LicenseFile=
OutputDir=$DistDir
OutputBaseFilename=GlassTraxAPIAgent-$Version-Setup
...
Name: "{group}\{#MyAppName}"; Filename: "{app}\python\pythonw.exe"; Parameters: "-m agent.cli --tray"; WorkingDir: "{app}"; IconFilename: "{app}\agent\icons\icon_running.ico"; Comment: "Start GlassTrax API Agent in system tray"
; Start Menu - Console Mode
Name: "{group}\{#MyAppName} (Console)"; Filename: "{app}\python\python.exe"; Parameters: "-m agent.cli --console"; WorkingDir: "{app}"; IconFilename: "{app}\agent\icons\icon_running.ico"; Comment: "Start GlassTrax API Agent in console mode"
; Start Menu - Uninstall
Name: "{group}\Uninstall {#MyAppName}"; Filename: "{uninstallexe}"
; Desktop Icon (optional)
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\python\pythonw.exe"; Parameters: "-m agent.cli --tray"; WorkingDir: "{app}"; IconFilename: "{app}\agent\icons\icon_running.ico"; Tasks: desktopicon; Comment: "Start GlassTrax API Agent"

[Registry]
; Auto-start on Windows boot (optional)
Root: HKCU; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "GlassTraxAPIAgent"; ValueData: """{app}\python\pythonw.exe"" -m agent.cli --tray"; Flags: uninsdeletevalue; Tasks: autostart

[Run]
; Launch after install
Filename: "{app}\python\pythonw.exe"; Parameters: "-m agent.cli --tray"; WorkingDir: "{app}"; Description: "Launch GlassTrax API Agent"; Flags: nowait postinstall skipifsilent
...
begin
  ...
  begin
    AppDataPath := ExpandConstant('{userappdata}\GlassTrax API Agent');
    if DirExists(AppDataPath) then
    begin
      if MsgBox('Do you want to remove configuration and log files?' + #13#10 + #13#10 +
                'Location: ' + AppDataPath + #13#10 + #13#10 +
                'Click Yes to remove all settings (including API key).' + #13#10 +
                'Click No to keep settings for future reinstall.',
...
if (-not $SkipInstaller) {
    ...
    # Success banner
    $InstallerPath = Join-Path $DistDir "GlassTraxAPIAgent-$Version-Setup.exe"
    Write-Host ""
